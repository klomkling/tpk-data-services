...
namespace Tpk.DataServices.Server.Controllers
{
    [AuthorizeRequired]
    [ApiController]
    [Route("api/material-grades")]
    public class MaterialGradesController : TgControllerBase<MaterialGrade, MaterialGrade>
    {
        public MaterialGradesController(IServiceProvider serviceProvider, IMaterialGradeService materialGradeService,
            ILogger<MaterialGradesController> logger)
            : base(serviceProvider, materialGradeService, logger)
        {
        }
        ...
        [HttpPost]
        [ClaimRequired(RestrictItems.MaterialGrades, RestrictActions.Create)]
        public override async Task<IActionResult> InsertAsync([FromBody] MaterialGrade model)
        {
            return await base.InsertAsync(model);
        }

        [HttpPatch]
        [ClaimRequired(RestrictItems.MaterialGrades, RestrictActions.Update)]
        public override async Task<IActionResult> UpdateAsync([FromBody] MaterialGrade model)
        {
