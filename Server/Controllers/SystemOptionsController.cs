...
namespace Tpk.DataServices.Server.Controllers
{
    ...
    public class SystemOptionsController : TgControllerBase<SystemOption, SystemOption>
    {
        public SystemOptionsController(IServiceProvider serviceProvider, ISystemOptionService systemOptionService,
            ILogger<SystemOptionsController> logger)
            : base(serviceProvider, systemOptionService, logger)
        {
        }
        ...
        [HttpPost]
        [ClaimRequired(RestrictItems.SystemOptions, RestrictActions.Create)]
        public override async Task<IActionResult> InsertAsync([FromBody] SystemOption model)
        {
            return await base.InsertAsync(model);
        }

        [HttpPatch]
        [ClaimRequired(RestrictItems.SystemOptions, RestrictActions.Update)]
        public override async Task<IActionResult> UpdateAsync([FromBody] SystemOption model)
        {
            return await base.UpdateAsync(model);
        }

        [HttpDelete]
        [ClaimRequired(RestrictItems.SystemOptions, RestrictActions.Delete)]
