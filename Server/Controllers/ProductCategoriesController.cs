...
namespace Tpk.DataServices.Server.Controllers
{
    ...
    public class ProductCategoriesController : TgControllerBase<ProductCategory, ProductCategory>
    {
        public ProductCategoriesController(IServiceProvider serviceProvider,
            IProductCategoryService productCategoryService,
            ILogger<ProductCategoriesController> logger)
            : base(serviceProvider, productCategoryService, logger)
        {
        }
        ...
        [HttpPost]
        [ClaimRequired(RestrictItems.ProductCategories, RestrictActions.Create)]
        public override async Task<IActionResult> InsertAsync([FromBody] ProductCategory model)
        {
            return await base.InsertAsync(model);
        }

        [HttpPatch]
        [ClaimRequired(RestrictItems.ProductCategories, RestrictActions.Update)]
        public override async Task<IActionResult> UpdateAsync([FromBody] ProductCategory model)
        {
            return await base.UpdateAsync(model);
        }

        [HttpDelete]
        [ClaimRequired(RestrictItems.ProductCategories, RestrictActions.Delete)]
