...
namespace Tpk.DataServices.Server.Controllers
{
    ...
    public class CustomerAddressesController : TgControllerBase<CustomerAddress, CustomerAddress>
    {
        public CustomerAddressesController(IServiceProvider serviceProvider,
            ICustomerAddressService customerAddressService, ILogger<CustomerAddressesController> logger)
            : base(serviceProvider, customerAddressService, logger)
        {
        }
        ...
        [HttpGet("{id:int}")]
        [ClaimRequired(RestrictItems.CustomerAddresses, RestrictActions.View)]
        ...
        [HttpPost]
        [ClaimRequired(RestrictItems.CustomerAddresses, RestrictActions.Create)]
        public override async Task<IActionResult> InsertAsync([FromBody] CustomerAddress model)
        {
            return await base.InsertAsync(model);
        }

        [HttpPatch]
        [ClaimRequired(RestrictItems.CustomerAddresses, RestrictActions.Update)]
        public override async Task<IActionResult> UpdateAsync([FromBody] CustomerAddress model)
        {
            return await base.UpdateAsync(model);
        }

        [HttpDelete]
        [ClaimRequired(RestrictItems.CustomerAddresses, RestrictActions.Delete)]
