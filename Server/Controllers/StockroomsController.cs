...
namespace Tpk.DataServices.Server.Controllers
{
    ...
    public class StockroomsController : TgControllerBase<Stockroom, Stockroom>
    {
        public StockroomsController(IServiceProvider serviceProvider, IStockroomService stockroomService,
            ILogger<StockroomsController> logger)
            : base(serviceProvider, stockroomService, logger)
        {
        }
        ...
        [HttpPost]
        [ClaimRequired(RestrictItems.Stockrooms, RestrictActions.Create)]
        public override async Task<IActionResult> InsertAsync([FromBody] Stockroom model)
        {
            return await base.InsertAsync(model);
        }

        [HttpPatch]
        [ClaimRequired(RestrictItems.Stockrooms, RestrictActions.Update)]
        public override async Task<IActionResult> UpdateAsync([FromBody] Stockroom model)
        {
            return await base.UpdateAsync(model);
        }

        [HttpDelete]
        [ClaimRequired(RestrictItems.Stockrooms, RestrictActions.Delete)]
