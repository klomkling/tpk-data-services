...
namespace Tpk.DataServices.Server.Controllers
{
    [AuthorizeRequired]
    [ApiController]
    [Route("api/customer-order-lines")]
    public class CustomerOrderLinesController : TgControllerBase<CustomerOrderLineWithDetail, CustomerOrderLine>
    {
        public CustomerOrderLinesController(IServiceProvider serviceProvider,
            ICustomerOrderLineService customerOrderLineService, ILogger<CustomerOrderLinesController> logger)
            : base(serviceProvider, customerOrderLineService, logger)
        {
        }
        ...
        [HttpPost]
        [ClaimRequired(RestrictItems.CustomerOrderLines, RestrictActions.Create)]
        public override async Task<IActionResult> InsertAsync([FromBody] CustomerOrderLine model)
        {
            return await base.InsertAsync(model);
        }

        [HttpPatch]
        [ClaimRequired(RestrictItems.CustomerOrderLines, RestrictActions.Update)]
        public override async Task<IActionResult> UpdateAsync([FromBody] CustomerOrderLine model)
        {
            return await base.UpdateAsync(model);
        }

        [HttpDelete]
        [ClaimRequired(RestrictItems.CustomerOrderLines, RestrictActions.Delete)]
