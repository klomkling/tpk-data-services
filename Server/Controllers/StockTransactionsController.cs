...
namespace Tpk.DataServices.Server.Controllers
{
    ...
    public class StockTransactionsController : TgControllerBase<StockTransaction, StockTransaction>
    {
        public StockTransactionsController(IServiceProvider serviceProvider,
            IStockTransactionService stockTransactionService, ILogger<StockTransactionsController> logger)
            : base(serviceProvider, stockTransactionService, logger)
        {
        }
        ...
        [HttpPost]
        [ClaimRequired(RestrictItems.StockTransactions, RestrictActions.Create)]
        public override async Task<IActionResult> InsertAsync([FromBody] StockTransaction model)
        {
            return await base.InsertAsync(model);
        }

        [HttpPatch]
        [ClaimRequired(RestrictItems.StockTransactions, RestrictActions.Update)]
        public override async Task<IActionResult> UpdateAsync([FromBody] StockTransaction model)
        {
            return await base.UpdateAsync(model);
        }

        [HttpDelete]
        [ClaimRequired(RestrictItems.StockTransactions, RestrictActions.Delete)]
