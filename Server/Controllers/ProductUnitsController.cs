...
namespace Tpk.DataServices.Server.Controllers
{
    ...
    public class ProductUnitsController : TgControllerBase<ProductUnit, ProductUnit>
    {
        public ProductUnitsController(IServiceProvider serviceProvider, IProductUnitService productUnitService,
            ILogger<ProductUnitsController> logger)
            : base(serviceProvider, productUnitService, logger)
        {
        }
        ...
        [HttpPost]
        [ClaimRequired(RestrictItems.ProductUnits, RestrictActions.Create)]
        public override async Task<IActionResult> InsertAsync([FromBody] ProductUnit model)
        {
            return await base.InsertAsync(model);
        }

        [HttpPatch]
        [ClaimRequired(RestrictItems.ProductUnits, RestrictActions.Update)]
        public override async Task<IActionResult> UpdateAsync([FromBody] ProductUnit model)
        {
            return await base.UpdateAsync(model);
        }

        [HttpDelete]
        [ClaimRequired(RestrictItems.ProductUnits, RestrictActions.Delete)]
