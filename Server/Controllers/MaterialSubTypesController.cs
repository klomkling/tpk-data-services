...
namespace Tpk.DataServices.Server.Controllers
{
    [AuthorizeRequired]
    [ApiController]
    [Route("api/material-sub-types")]
    public class MaterialSubTypesController : TgControllerBase<MaterialSubType, MaterialSubType>
    {
        public MaterialSubTypesController(IServiceProvider serviceProvider,
            IMaterialSubTypeService materialSubTypeService, ILogger<MaterialSubTypesController> logger)
            : base(serviceProvider, materialSubTypeService, logger)
        {
        }
        ...
        [HttpPost]
        [ClaimRequired(RestrictItems.MaterialSubTypes, RestrictActions.Create)]
        public override async Task<IActionResult> InsertAsync([FromBody] MaterialSubType model)
        {
            return await base.InsertAsync(model);
        }

        [HttpPatch]
        [ClaimRequired(RestrictItems.MaterialSubTypes, RestrictActions.Update)]
        public override async Task<IActionResult> UpdateAsync([FromBody] MaterialSubType model)
        {
            return await base.UpdateAsync(model);
        }

        [HttpDelete]
        [ClaimRequired(RestrictItems.MaterialSubTypes, RestrictActions.Delete)]
