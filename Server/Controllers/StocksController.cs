...
namespace Tpk.DataServices.Server.Controllers
{
    ...
    public class StocksController : TgControllerBase<StockWithDetail, Stock>
    {
        private readonly IStockService _stockService;

        public StocksController(IServiceProvider serviceProvider, IStockService stockService,
            ILogger<StocksController> logger)
            : base(serviceProvider, stockService, logger)
        {
        ...
        {
            return base.SearchAsync(columns, searchStrings);
        }
        ...
        public override async Task<IActionResult> GetAllAsync(DataSourceLoadOptions loadOptions)
        {
            try
            {
                var orderColumns = $"{nameof(StockWithDetail.ProductCode)}, {nameof(StockWithDetail.StockroomName)}, " +
                                   $"{nameof(StockWithDetail.StockLocation)}, {nameof(StockWithDetail.PackageCode)}, " +
                                   $"{nameof(StockWithDetail.ReceivedDate)}";

                var condition = $"{nameof(StockWithDetail.Quantity)} > {nameof(StockWithDetail.UsedQuantity)}";

                var collection =
                    await _stockService.GetAllAsync<StockWithDetail>(IsAdmin == false, orderColumns, condition);
        ...
        [HttpPost]
        [ClaimRequired(RestrictItems.Stocks, RestrictActions.Create)]
        public override async Task<IActionResult> InsertAsync([FromBody] Stock model)
        {
            return await base.InsertAsync(model);
        }

        [HttpPatch]
        [ClaimRequired(RestrictItems.Stocks, RestrictActions.Update)]
        public override async Task<IActionResult> UpdateAsync([FromBody] Stock model)
        {
            return await base.UpdateAsync(model);
        }

        [HttpDelete]
        [ClaimRequired(RestrictItems.Stocks, RestrictActions.Delete)]
