...
namespace Tpk.DataServices.Server.Controllers
{
    [AuthorizeRequired]
    [ApiController]
    [Route("api/customer-contacts")]
    public class CustomerContactsController : TgControllerBase<CustomerContactWithDetail, CustomerContact>
    {
        public CustomerContactsController(IServiceProvider serviceProvider,
            ICustomerContactService customerContactService, ILogger<CustomerContactsController> logger)
            : base(serviceProvider, customerContactService, logger)
        {
        }
        ...
        [HttpPost]
        [ClaimRequired(RestrictItems.CustomerContacts, RestrictActions.Create)]
        public override async Task<IActionResult> InsertAsync([FromBody] CustomerContact model)
        {
            return await base.InsertAsync(model);
        }

        [HttpPatch]
        [ClaimRequired(RestrictItems.CustomerContacts, RestrictActions.Update)]
        public override async Task<IActionResult> UpdateAsync([FromBody] CustomerContact model)
        {
            return await base.UpdateAsync(model);
        }

        [HttpDelete]
        [ClaimRequired(RestrictItems.CustomerContacts, RestrictActions.Delete)]
