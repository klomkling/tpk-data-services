...
namespace Tpk.DataServices.Server.Controllers
{
    [AuthorizeRequired]
    [ApiController]
    [Route("api/customer-product-packages")]
    public class CustomerProductPackagesController : TgControllerBase<CustomerProductPackageWithDetail, CustomerProductPackage>
    {
        public CustomerProductPackagesController(IServiceProvider serviceProvider,
            ICustomerProductPackageService customerProductPackageService,
            ILogger<CustomerProductPackagesController> logger)
            : base(serviceProvider, customerProductPackageService, logger)
        {
        }
        ...
        [HttpPost]
        [ClaimRequired(RestrictItems.CustomerProductPackages, RestrictActions.Create)]
        public override async Task<IActionResult> InsertAsync([FromBody] CustomerProductPackage model)
        {
            return await base.InsertAsync(model);
        }

        [HttpPatch]
        [ClaimRequired(RestrictItems.CustomerProductPackages, RestrictActions.Update)]
        public override async Task<IActionResult> UpdateAsync([FromBody] CustomerProductPackage model)
        {
            return await base.UpdateAsync(model);
        }

        [HttpDelete]
        [ClaimRequired(RestrictItems.CustomerProductPackages, RestrictActions.Delete)]
