...
{
    public class SupplierProductPackageEditContextValidator : AbstractValidator<SupplierProductPackageEditContext>
    {
        private readonly IApiService _apiService;

        public SupplierProductPackageEditContextValidator(IApiService apiService)
        {
        ...
        private async Task<bool> BeUniqueUnitAsync(SupplierProductPackageEditContext model, int packageTypeId,
            CancellationToken cancellationToken)
        {
            var req = new ValidationRequest
            {
                ...
                {
                    model.CreateValidationColumn(nameof(model.SupplierProductId), model.SupplierProductId),
                    model.CreateValidationColumn(nameof(model.PackageTypeId), packageTypeId)
                }
            };

            var isUnique = await _apiService.IsUniqueValueAsync(req, "api/supplier-product-packages/unique-validation");
